...
#define WGL_TYPE_RGBA_RAB             0x202B
...
bool InitializeOpenGL(HWND hwnd, int screenWidth, int screenHeight, float screenDepth, float screenNear, bool vsync)
{
    ...
    attributeListInt[15] = WGL_TYPE_RGBA_RAB;

    // Support for a 8bit stencil buffer
    attributeListInt[16] = WGL_STENCIL_BITS_ARB;
    attributeListInt[17] = 8;

    // Null terminates the attribute list
    ...
    // Set the polygon winding to frong facing for the left handed system
    glFrontFace(GL_CW);
...
bool InitializeExtensions(HWND hwnd)
{
    ...
    // Release the temporary rendering context
    wglMakeCurrent(NULL, NULL);
...
void OutputShaderErrorMessage(HWND hwnd, unsigned int shaderId, const char* shaderFilename)
{
    ...
    // Get the size of the string containing the information log for the failed shader
    ...
    // Pop a message up on the screen to notify the user to check the text file for
...
void OutputLinkerErrorMessage(HWND hwnd, unsigned int programId)
{
    ...
    // Increments the size by one to handle also the null terminator
    logSize++;
    ...
    // Pop a message up on the screen to notify user to check the text file
    MessageBox(hwnd, _T("Error compiling linker. Check linker-error.txt for detailed message"), _T("Linker Error"), MB_OK);
}
...
void CalculateCameraPosition()
{
    ...
    // Transform the lookAt and up vector by the rotation matrix so the view is correct
...
void Draw()
{
    ...
    // Set the color shader as the current shader program and set the matrices that
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
    ...
    hWnd = CreateWindowEx(WS_EX_APPWINDOW,
        ...
        0, // x-pos of the window
        0, // y-pos of the window
